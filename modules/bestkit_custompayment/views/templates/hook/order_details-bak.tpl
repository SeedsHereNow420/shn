<div class="bestkit_custompayment_wrapper">
	...
	<div>{l s='- The total amount of your order comes to:' mod='bestkit_custompayment'}<span id="amount" class="price"> {displayPrice price=$total}</span> {l s='(tax incl.)' mod='bestkit_custompayment'}</div>
		
	<div>{l s='- The fee/discount of your order comes to:' mod='bestkit_custompayment'}<span id="amount" class="price"> {displayPrice price=$custom_payment_fee}</span></div>
...
<table>
	<tr class="item bestkit_custompayment_row">
		...
		<td colspan="4">
			<span class="custom_payment_fee price">{displayPrice price=$custom_payment_fee}</span>
		</td>
	</tr>
</table>
