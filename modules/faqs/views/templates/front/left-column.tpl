      ...
      <div class="block_content list-block">
        <ul class="categories">
          {foreach from=$infos['faqCategories'] item=faqCategory}
            <li>
              <a class="category_name name_{$faqCategory['id_gomakoil_faq_category']|escape:'htmlall':'UTF-8'} change_item"  href="{$infos['faqUrl']|escape:'htmlall':'UTF-8'}{$faqCategory['link_rewrite']|escape:'htmlall':'UTF-8'}.html">{$faqCategory['name']|escape:'htmlall':'UTF-8'}</a>
            </li>
          {/foreach}
        </ul>
      </div>
