...
<div class="easy_icon_wrap flex_container {if $element.st_el_icon_with_text=='2_4'} flex_start {else} pad_b1 {/if}">
	<em class="{$element.st_el_icon} fs_xl easy_icon color_444"><span class="unvisible">&nbsp;</span></em>
	<div class="flex_child">
        {if $element.st_el_icon_with_text=='2_3' && $element.st_el_sub_header}<div class="easy_sub_header">{$element.st_el_sub_header}</div>{/if}
		{if isset($element.st_el_header) && $element.st_el_header}
            {if $element.st_el_url}
                <a href="{$element.st_el_url}" class="fs_lg color_444 easy_header" rel="nofollow" title="{$element.st_el_header}">{$element.st_el_header}</a>                
            {else}
                <div class="fs_lg color_444 easy_header">{$element.st_el_header}</div>
            {/if}
        {/if}
		{if $element.st_el_icon_with_text!='2_3' && $element.st_el_sub_header}<div class="easy_sub_header">{$element.st_el_sub_header}</div>{/if}
	</div>
