...
<head>
    ...
    $(window).load(function()
    {
        ...
        function loadPageData(pagenumber, delfilename)
        {
            ...
            {//send a delete request with the page load
                ajaxurl = "<?php echo Context::getContext()->link->getAdminLink('AdminLayerSliderMedia') ?>&action=listImages&d="+window.parent.imgpath+"&p="+pagenumber+"&del="+encodeURIComponent(delfilename);
            }
            else
            {//else just get the page
                ajaxurl = "<?php echo Context::getContext()->link->getAdminLink('AdminLayerSliderMedia') ?>&action=listImages&d="+window.parent.imgpath+"&p="+pagenumber;
            }
        ...
        $("a.thumb").hover(function(e){
            $("#thumb").remove();
            $("body").append("<div id=\"thumb\"><img src=\"<?php echo LS_MEDIA_PATH ?>encodeexplorer.php?thumb="+ $(this).attr("href") +"\" alt=\"Preview\" \/><\/div>");
            positionThumbnail(e);
            $("#thumb").fadeIn("medium");
        },
        function(){
    ...
    <?php $css_uri = __PS_BASE_URI__.'modules/layerslider/views/css/mediamanager' ?>
    ...
    <?php $js_uri = __PS_BASE_URI__.'modules/layerslider/views/js/mediamanager' ?>
...
<body>
    ...
    <div class="tab-content">
        <div class="tab-pane<?php echo ($tabno == 1) ? ' active' : '' ?>" id="home">
            <div id="upload-img">
                <form name="upload" class="fileuploadsingle" action="<?php echo Context::getContext()->link->getAdminLink('AdminLayerSliderMedia') ?>&action=uploadFile" method="POST" enctype="multipart/form-data">
