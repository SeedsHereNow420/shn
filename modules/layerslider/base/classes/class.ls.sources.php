...
class LsSources
{
    ...
    public static function addDemoSlider($path)
    {
        ...
        if (! empty($slidersPath)) {
            foreach ($slidersPath as $key => $path) {
                ...
                // Get skin info (if any)
                if (file_exists($path.'/info.json')) {
                    $sliders[$handle]['info'] = Tools::jsonDecode(Tools::file_get_contents($path.'/info.json'), true);
                ...
                if (file_exists($path.'/preview.png')) {
                    $url = ls_content_url() . str_replace(realpath(LS_CONTENT_DIR), '', $path).'/preview.png';
    ...
    {
        unset(self::$sliders[ Tools::strtolower($handle) ]);
    }
    ...
    {
        return self::$sliders[ Tools::strtolower($handle) ];
    }
    ...
    {
        return self::$sliders[ Tools::strtolower($handle) ]['dir'] . DIRECTORY_SEPARATOR;
    }
