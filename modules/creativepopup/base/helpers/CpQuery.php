...
defined('CP_QUERY') or define('CP_QUERY', true);
...
class CpQuery
{
    ...
    public function __construct($html = null)
    {
        if ($html) {
            ...
            $this->dom->loadHTML(self::utf8Decode(preg_replace('~>\s+<~', '><', trim($html))));
    ...
    public function append($html)
    {
        $doc = new DOMDocument();
        $doc->encoding = 'utf-8';
        $doc->loadHTML(self::utf8Decode('<div>'.preg_replace('~>\s+<~', '><', trim($html)).'</div>'));
