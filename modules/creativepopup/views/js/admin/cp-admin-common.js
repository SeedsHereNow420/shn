...
var lsLogo = {
	...
	append: function( to, animated ){
		...
		to.addClass( 'cp-logo' ).attr('data-l10n-importing', CP_l10n.SLImporting );

		if ( animated ){
			to.addClass( 'cp-logo-animated' );
		}

		jQuery( '<div class="cp-logo-setheight"></div><div class="cp-logo-layer"><div class="cp-logo-inner"><div class="cp-logo-color"></div></div></div><div class="cp-logo-layer"><div class="cp-logo-inner"><div class="cp-logo-color"></div></div></div><div class="cp-logo-layer"><div class="cp-logo-inner"><div class="cp-logo-color"></div></div></div>' ).appendTo( to );
	},
	...
	remove: function( from ){
		...
		from.removeClass( 'cp-logo cp-logo-animated').empty();
	}
};
...
(function( $ ) {
	...
	$.fn.customCheckbox = function() {
		return this.each(function() {
			...
			var $rep = $('<a href="#"><span></span></a>').addClass('cp-checkbox').insertAfter(this);
	...
	$.fn.kmAccordion = function() {
		...
		this.children().each(function(){
			...
			$aHead.on('click', function(event){
				...
				if($(event.target).is('.dashicons, .cp-checkbox, .cp-checkbox > span')) {
					return;
				}
...
var CP_CodeMirror = {
	...
	init : function(settings) {
		...
		jQuery('.cp-codemirror').each(function() {
			...
			if( jQuery(this).closest('.cp-callback-box').length ) {
...
jQuery(function($) {
	...
	var lsScreenOptionsActions = {
		...
		init : function() {
			...
			// Form submit
			$(document).on('submit', '#cp-screen-options-form', function(e) {
				e.preventDefault(); lsScreenOptionsActions.saveSettings(this, true);
			});

			// Checkboxes
			$(document).on('click', '#cp-screen-options-form input:checkbox', function() {
	...
	var lsSlideUnder = {
		...
		init : function(){
			...
			$(document).on('click', '[data-cp-su]', function() {
				//if( $(this).parent().find('.cp-su').length == 0 ){
		...
		create : function($el){
			...
			$su.addClass('cp-su');
			$sui.addClass('cp-su-inner');
			$suc.addClass('cp-su-content');
			...
			$suc.append( $el.siblings('.cp-su-data').html() );
		},

		open : function( $el ){

			if( !$el.parent().find( '.cp-su' ).length ){
				lsSlideUnder.create( $el );
			}

			$su = $el.parent().find( '.cp-su' );
			$sui = $el.parent().find( '.cp-su-inner' );

			if( $su.hasClass( 'cp-su-opened') ){
				return;
			}

			$su.addClass( 'cp-su-opened' );
	...
	var lsUIDependencies = {
		...
		init : function() {
			...
			$(document).on('change input click', '[data-toggleitems], [data-showitems], [data-hideitems]', function( event ) {
				...
				if( event.type === 'click' && $el.is('select,input,textarea,.cp-checkbox') ) {
					return;
				}
				...
				if( $el.data('showitems') ) {
					jQuery( $el.data('showitems') ).addClass('cp-hidden');
				}

				if ( $el.data('hideitems') ) {
					jQuery( $el.data('hideitems') ).removeClass('cp-hidden');
				}
				...
				if( $el.data('toggleitems') ) {
					...
					if( $el.data('opened') ) {
						...
						$targets.addClass('cp-hidden');
					} else {
						$el.data('opened', true);
						$targets.removeClass('cp-hidden');
					}

					// toggle other show advanced settings checkbox
					jQuery('.cp-checkbox[data-toggleitems]').filter($el.data('opened') ? '.off' : '.on').click();
				}
			});
		},
	};
	...
	// Screen options
	$('#cp-screen-options, #cp-guides').children(':first-child').appendTo('#screen-meta');
	$('#cp-screen-options, #cp-guides').children(':last-child').appendTo('#screen-meta-links');
	...
	// Skin Editor
	if(~location.href.indexOf('AdminCreativePopupSkin')) {
		$('select[name="skin"]').change(function() {
			location.href = location.href.replace(/skin=[^&]+/, 'skin=' + $(this).children(':selected').val());
		});
	}


	// Checkbox event
	$(document).on('click', '.cp-checkbox', function(e){
		e.preventDefault();
		...
		// Trigger events
		$(el).trigger('change');
		$('#cp-layers').trigger( $.Event('click', { target : el } ) );
		$(document).trigger( $.Event('click', { target : el } ) );
	});


	// Share sheet
	$('#cp-share-template .inner a').click(function(e) {
		e.preventDefault();
	...
	$('#cp-share-template h3 a').click(function(e) {
		e.preventDefault();
		$('#cp-share-template, .cp-overlay').remove();
	});
});
