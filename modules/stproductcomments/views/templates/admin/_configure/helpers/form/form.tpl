...
{block name="field"}
	...
	{elseif $input.type == 'pc_images'}
		<div class="col-lg-9">
			<input name="{$input.name}" id="pc_images" type="hidden" value="{$input.values.str}" />
			<ul class="pc_images_list clearfix">
				{foreach $input.values.data as $img}
					<li>
						<img src="{$img.url}" alt="{$img.name}">
						<a href="javascript:;" class="pc_image_item" data-image="{$img.name}">{l s='Remove' mod='stproductcomments'}</a>
					</li>
				{/foreach}
			</ul>
		</div>
