...
<li class="tree-folder">
	<span class="tree-folder-name{if isset($node['disabled']) && $node['disabled'] == true} tree-folder-name-disable{/if}">
		...
		<label class="tree-toggler">{$node['name']}{if isset($node['selected_childs']) && (int)$node['selected_childs'] > 0} {l s='(%s selected)' d='Shop.Theme.Transformer' sprintf=$node['selected_childs']}{/if}</label>
	</span>
	<ul class="tree">
		{$children}
	</ul>
