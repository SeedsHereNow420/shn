...
$error=$prismpay->displayError($this->l("There was error processing payment: Invalid Response.\n You can return to Checkout page and make another try"));
    ...
    if (!empty($cart_products)) {
        foreach ($cart_products as $key => $cart_product) {
			$cart_for_log[$key]['product_id']=$cart_product['id_product'];
			$cart_for_log[$key]['quantity']=$cart_product['cart_quantity'] ;
...
exit(); */
 ...
 $card_filled=false;
if($stock_error==false && $cart_empty==false && Tools::getValue('pc_cardholder_name') && Tools::getValue('pc_cardnum')&& Tools::getValue('pc_exp_date')&& Tools::getValue('pc_cvv')  ){ 
$card_filled=true;
        $data = array_merge($data, array(
...
$result = $client->CreditSale_Soap(array('creditCardSale'=>array('ServiceSecurity'=>$service_security,'TokenData'=>$token_data,'TransactionData'=>$transaction_data)) );
  ...
  $client_request_xml_log=$client->__getLastRequest();
/*   $xml_elem = new SimpleXMLElement($client_request_xml);
  $client_request_log=$xml_elem->__toString(); */
  
  $token_data_log=json_encode($token_data);
  $transact_data_log=json_encode($transaction_data);
  $transaction_time_log=date('Y-m-d H:i:s');
 $response_log=json_encode($result);
 //$cart_products_log=json_encode($cart_products);
 $cart_products_for_log=json_encode($cart_for_log);
 $cart_id_log=$cart->id ;
 $order_id_log=(int)$prismpay->currentOrder;
// $customer ;
  Db::getInstance()->execute("INSERT INTO `" . _DB_PREFIX_ . "prismpay_log` (`ppl_response`, `ppl_cart_id`, `ppl_cart`, `ppl_transaction_time`, `ppl_user_id`, `ppl_request_string`, `ppl_token_data`, `ppl_transaction_data`) VALUES (  '". pSQL($response_log) . "','".pSQL($cart_id_log) . "','".pSQL($cart_products_for_log) . "','".pSQL($transaction_time_log) . "','".pSQL($customer->id) . "','".$client_request_xml_log. "','".pSQL($token_data_log) . "','".pSQL($transact_data_log)."') ");
...
elseif($cart_empty===true){
 $error=$prismpay->displayError($this->l('Your cart is empty.Please shop for some items and try again'));
}
