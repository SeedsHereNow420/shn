...
{if isset($view_header) && $view_header && isset($root) && $root}
	...
	<div class="wrap_snap_category pull-right">
		<input type="checkbox" id="bind_child" /> {l s='Snap the child categories' mod='dgridproducts'}<br />
		<input type="checkbox" id="search_default_categories"{if Tools::getValue('search_default_categories')} checked{/if} /> {l s='Only default' mod='dgridproducts'}
	</div>
{/if}
{if isset($categories)}
...
{elseif $root}
	{l s='Not categories' mod='dgridproducts'}
{/if}