...
function TreeCustom(tree_selector, tree_header_selector)
{
    ...
    this.search = function (string) {
        var elements = this.ts.find('.tree_input[data-search*="'+string.replace('"', '')+'"]').not(':checked');
        if (!elements.length)
            return '';
        var html_response = '';
    ...
    this.init = function()
    {
        ...
        this.th.find('.select_search').live('click' ,function (e) {
            ...
            if (self.checkAssociatedCategory(id))
            {
                ...
                $(this).closest('.result_search').remove();
                self.th.find('.search_category').val('');
                var select_input = $('.block_category_tree.tree_root:first input[value='+id+']');
                select_input.attr('checked', 'checked');
                select_input.parent().addClass("tree_selected");
                select_input.parents('li.tree_item').each(function() {
