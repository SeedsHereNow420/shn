...
jQuery(function($){
    ...
    $('.st_delete_image').click(function(){
        ...
        var id = self.data('id');
        $.getJSON(currentIndex+'&token='+token+'&configure=stowlcarousel&id_st_owl_carousel='+id+'&id_lang='+id_lang+'&act=delete_image&ts='+new Date().getTime(),
            function(json){
                if(json.r) {
...
function troggleItemsBox()
{
	if( $("input[name='templates']:checked").val() == 3 )
	{
		$('#items_box').show().parents('.form-group').show();
	}
	else{
		$('#items_box').parents('.form-group').hide();
	}
}